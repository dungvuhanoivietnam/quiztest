...
class ItemView : FrameLayout {
    ...
    private var boxBackgroundColorActive: Drawable? = null
    private var boxBackgroundColorInactive: Int = 0
    private var boxBackgroundColorSuccess: Drawable? = null
    private var boxBackgroundColorError: Drawable? = null
    ...
    private fun generateViews(styles: TypedArray) {
        ...
        boxBackgroundColorActive = context.resources?.getDrawable(R.drawable.bg_active_code_view)

        boxBackgroundColorInactive = styles.getResourceId(
            R.styleable.OtpTextView_otp_box_background_inactive,
            boxBackgroundColor
        )
        boxBackgroundColorSuccess = context.resources?.getDrawable(R.drawable.bg_active_code_view)

        boxBackgroundColorError = context.resources?.getDrawable(R.drawable.bg_error_code_view)

        barActiveColor = styles.getColor(
            R.styleable.OtpTextView_bar_active_color,
            ResourcesCompat.getColor(context.resources, R.color.color_4CCEFE, null)
        )
        barInactiveColor = styles.getColor(
            R.styleable.OtpTextView_bar_inactive_color,
            ResourcesCompat.getColor(context.resources, R.color.color_B8BDC2, null)
        )
        barErrorColor = styles.getColor(
            R.styleable.OtpTextView_bar_error_color,
            ResourcesCompat.getColor(context.resources, R.color.color_DC2323, null)
        )
    ...
    fun setViewState(state: Int) {
        when (state) {
            ACTIVE -> {
                ...
                this.setBackgroundDrawable(boxBackgroundColorActive)
            }
            INACTIVE -> {
            ...
            ERROR -> {
                ...
                this.setBackgroundDrawable(boxBackgroundColorError)
            }
            SUCCESS -> {
                view?.setBackgroundColor(barSuccessColor)
                this.setBackgroundDrawable(boxBackgroundColorSuccess)
            }
            else -> {
            }
        }
